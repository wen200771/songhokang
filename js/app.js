...
function isFavoritesView() {
    return (
        currentView === "favorites" ||
        document.body.classList.contains("view-favorites")
    );
}
...
function buildDummyCoupons() {
    ...
    for (let i = 1; i <= 50; i++) {
        ...
        const storeType =
            storeTypes[category][
                Math.floor(Math.random() * storeTypes[category].length)
            ];
        ...
        coupons.push({
            ...
            phone: `02-2${String(i).padStart(3, "0")}-${String(i * 10).padStart(
                4,
                "0"
            )}`,
        });
        ...
        if (grid) {
            ...
            grid.querySelectorAll(".category-chip").forEach((item) => {
                item.addEventListener("click", () => {
                    const categoryText = item.querySelector(
                        ".category-chip-text"
                    ).textContent;
                    const isSelected = selectedTags.some(
                        (tag) => tag.text === categoryText
                    );
                    if (isSelected) {
...
function renderCoupons(couponsToRender = coupons) {
    ...
    if (isFavoritesView()) {
        ...
        couponsToRender = couponsToRender.filter((coupon) =>
            favoriteIds.has(coupon.id)
        );
    ...
    couponsToRender.forEach((coupon, index) => {
        ...
        card.addEventListener("click", (ev) => {
            const t = ev.target;
            if (
                t.closest('[data-action="quick-fav"]') ||
                t.closest('[data-action="remove-fav"]')
            ) {
                // 點擊愛心或移除收藏不開啟詳情
                return;
            }
        ...
        card.innerHTML = `
            ...
            coupon.title
        }" class="coupon-image" loading="lazy" decoding="async" width="800" height="1200">
                    </picture>
                    <button class="fav-btn ${
                        isFav ? "active" : ""
                    }" data-action="quick-fav" data-id="${coupon.id}">${
            isFav ? "❤" : "♡"
        }</button>
...
async function openModal(coupon) {
    try {
        ...
        document.getElementById("offerDescription").textContent =
            coupon.description;
        ...
        if (res.ok) {
            ...
            if (detailedCoupon.terms) {
                document.getElementById("offerDescription").textContent =
                    detailedCoupon.terms;
            }
            if (detailedCoupon.discount_type && detailedCoupon.discount_value) {
                const discountText = getDiscountText(
                    detailedCoupon.discount_type,
                    detailedCoupon.discount_value
                );
                document.getElementById("offerUsage").textContent =
                    discountText;
            }
...
function generateUsageProof(coupon, usageResult = {}) {
    ...
    proofModal.style.cssText =
        "display: flex; z-index: 10002; background: rgba(0,0,0,0.8);";

    const verificationCode =
        usageResult.verification_code ||
        `SHK-${coupon.id}-${Date.now().toString().slice(-6)}`;
                ...
                <div class="modal-content" style="max-width: 450px; background: linear-gradient(135deg, #FCA311, #F79009); color: white; border: none;">
                    ...
                    <div style="padding: 40px 28px; text-align: center;">
                        ...
                        <div style="font-size: 12px; opacity: 0.8; margin-bottom: 16px;">
                            憑證有效期至：${new Date(expiresAt).toLocaleString(
                                "zh-TW"
                            )}
                        </div>
                        `
...
async function toggleFavorite(coupon) {
    if (isLoggedIn()) {
        try {
            ...
            if (isFavNow) {
                await fetch(`api/favorites/${coupon.id}`, {
                    method: "DELETE",
    ...
    document
        .querySelectorAll(`[data-action="quick-fav"][data-id="${coupon.id}"]`)
        .forEach((btn) => {
            ...
            if (Array.isArray(items) && items.length > 0) {
                ...
                const searchResults = items.map((item, idx) => ({
                    ...
                    discount: item.discount_value
                        ? `${item.discount_value}${
                              item.discount_type === "percentage" ? "%" : "元"
                          } OFF`
                        : "特價優惠",
...
// 側邊欄展開/關閉初始化（移除 hover 自動展開，只提供關閉鈕）
...
function updateSidebarCounts() {
    ...
    if (historyData.length > 0) {
        historyCountEl.textContent =
            historyData.length > 99 ? "99+" : historyData.length;
...
function loadCurrentSettings() {
    ...
    document.getElementById("showCounts").checked =
        settings.showCounts !== false;
    document.getElementById("autoExpand").checked =
        ...
        try {
            ...
            if (switchDataStr) {
                ...
                if (switchData.isolatedSession) {
                    ...
                    localStorage.setItem(
                        pageSwitchKey,
                        JSON.stringify(switchData.switched_from_admin)
                    );
                }
                ...
                setTimeout(() => {
                    showSuccessMessage(
                        `🔄 已切換到用戶「${switchData.user.username}」\n✅ 頁面隔離模式：其他頁面會話不受影響\n\n點擊右上角👤圖標可進入個人中心`
                    );
                }, 500);
...
function showAdminSwitchStatus() {
    if (isAdminSwitchedSession()) {
        try {
            ...
            const switchTime = switchInfo.switched_at
                ? new Date(switchInfo.switched_at).toLocaleString()
                : "未知";
            statusDiv.innerHTML = `
                        🔄 管理員切換模式 | 目前身份: ${
                            getUser()?.username ||
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.addEventListener("click", (e) => {
        if (!e.target.closest("#roleMenu")) roleMenu.classList.remove("active");
    });
    // 點角色項目（僅對有data-role屬性的元素生效）
    document
        .querySelectorAll("#roleMenu .role-item[data-role]")
        .forEach((btn) =>
    ...
    document.querySelectorAll(".trending-tag").forEach((item) => {
        item.addEventListener("click", () => {
            ...
            // 檢查是否已選中
            const isSelected = selectedTags.some(
                (selectedTag) => selectedTag.text === tag
            );
...
// 後備機制：若腳本載入時 DOM 已就緒，主動觸發初始化一次
if (document.readyState !== "loading" && !window.__appInitialized) {
...
function updateLoginUI() {
    ...
    if (isLoggedIn()) {
        ...
        if (role === "customer") {
            ...
            document
                .querySelectorAll("#roleMenu .role-item")
                .forEach((item) => {
...
async function applyAdvancedFilters() {
    ...
    // 到期時間篩選
    const expiryFilter = document.querySelector(
        'input[name="expiry"]:checked'
    ).value;
    if (expiryFilter !== "all") {
        const now = new Date();
        filteredCoupons = filteredCoupons.filter((coupon) => {
        ...
        case "expiry":
            filteredCoupons.sort((a, b) => {
                const dateA = a.expiry
                    ? new Date(a.expiry.replace(/\//g, "-"))
                    : new Date("2099-12-31");
                const dateB = b.expiry
                    ? new Date(b.expiry.replace(/\//g, "-"))
                    : new Date("2099-12-31");
                return dateA - dateB;
            });
            break;
...
async function onLoginSubmit(e) {
    ...
    try {
        ...
        if (json.success) {
            ...
            if (role === "customer") {
                showSuccessMessage(
                    `歡迎回來，${json.data.user?.username || "會員"}！`
                );
                closeAuth();
                // 一般會員留在前台，不跳轉
                return;
            ...
            showSuccessMessage(json.message || "登入失敗");
        }
    } catch (err) {
...
async function onRegisterSubmit(e) {
    ...
    if (!username || !email || !password) {
        showSuccessMessage("請完整填寫用戶名、電子郵件和密碼");
        return;
    }
    if (password !== password2) {
        showSuccessMessage("兩次密碼不一致");
        return;
    }
    if (role === "vendor" && !company_name) {
        showSuccessMessage("廠商註冊請填寫公司名稱");
        return;
    }
    try {
        const body = {
        ...
        if (json.success) {
            ...
            const details = json.details
                ...
                : "";
            showSuccessMessage(
                (json.message || "註冊失敗") + (details ? "：" + details : "")
            );
        }
    } catch (err) {
...
document.addEventListener("click", (e) => {
    if (
        ...
        e.preventDefault();
        toggleFavorite(currentCoupon).then(() => {
            syncModalFavoriteButton();
            updateFavCount();
        });
    }
